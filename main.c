...
{
    /* configura la placa pasando el n�mero de canales, el entero por el que se divide la freq de adquisici�n m�xima.
    qFreqAdq 1, 2,3,.......,2^16
...
void *raw_data_writer(void *n)
{
    ...
    while (salir == 0)
    { /// buscar mejor condicion de salida

        WaitForSingleObject(callback.semConsumidor[0], INFINITE); //espero si no hay datos

        if (callback.ovr)
        { ///OVR CHECK. Si no puedo seguir escribiendo, copio todos los datos del buffer.
            it_ovr++;
            if (it_ovr == CHUNKS - 1)
            { ///si ya di toda la vuelta, termino
                break;
            }
            printf("Hay overrun \n");
        }
        ...
        if (!infinite_daq && shots_totales >= nShots)
        { /// deja de adqurir cuando llego hasta aca (puede haber adquirido mas shots que shot lim). puede guardar m?s datos que shot_lim (si no es multiplo de shots_per_chunk).
            break;
        }

        if (current_chunk == CHUNKS)
        { //reinicio current_chunk
            current_chunk = 0;
        }
    }
...
void *procesa_STD(void *n)
{
    ...
    { ///wait podr�a estar aca
        ...
        for (i = 0; i < bins * (window_time - 1); ++i)
        { ///calcula ventana-1
            mean_w[i % bins] += (float)(chunk_actual[i * nCh]);
        }

        for (i = bins * (window_time - 1); i < bins * window_time; ++i)
        { ///termina ventana
        ...
        for (i = bins * window_time; i < bins * nShotsChk; ++i)
        { ///resta y suma el apropiado a la ventana calculada
...
void *procesa_Monitoreo(void *n)
{
    ...
    // AGREGA MARCO PARA VER ESTABILIDAD DE LA ENERG�A DE LOS PULSOS AMPLIFICADOS
