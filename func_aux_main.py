...
def data_gen_parameters(source_DAS,time_stamp_DAS):
    ...
    if source_DAS:
        ...
        # argv[1] es el tamaño del vector moving average
        # argv[2] es el tamaño de la venta utilizada en el moving av
        # la cantidad de bines reales es bins_plot
        # argv[3] es qFreq
        dtype_size=4 #tamanio del tipo de dato que viene por el pipe

        if time_stamp_DAS:
            now=datetime.datetime.now().strftime("%Y-%m-%d__%H_%M_%S")
            ts_file=open('timestamp_log/'+now+'.ts','a')
